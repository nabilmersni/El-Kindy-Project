...
import { deleteEvent, getEventById, getallEvents } from "../../Services/apiEvent";
import { Link } from 'react-router-dom';
import EventDetailsCard from "./eventDetails";
...
const EventCard = ({ data }) => {
  ...
  //       console.log('API Response:', eventsResult);
  ...
  useEffect(() => {
    if (data) {
      const fetchEvents = async () => {
          ...
          const fetchedEvent = await getEventById(data._id); // Assuming data.id contains the quiz ID
          console.log('Image URL:', `http://localhost:3000/upload-directory/${fetchedEvent.EventImage}`);
  ...
  const handleDelete = async (eventId) => {
    try {
      ...
      const updatedEvents = eventList.filter((event) => event._id !== eventId);
  ...
  // const imagePath = `http://localhost:3000/upload-directory/${data.EventImage}`;
  ...
  return (
    <div>
      ...
      <div key={data._id} className="dash__courses__list__card">
          ...
          <div className="dash__courses__list__card-content-btns">
            ...
            <div className="dash__courses__list__card-content-btns-list">
              ...
              {/* <Link to={`/event-details/${event._id}`}>
                    <button className="dash__courses__list__card-content-btns-list__show">
              ...
              <Link to={`/dash-admin-Update-new-event/${data._id}`}
                className="dash__courses__list__card-content-btns-list__show"
                onClick={(e) => {
                  if (!data || !data._id) {
                    e.preventDefault(); // Prevent navigation if data is not available
                    return;
                  }
