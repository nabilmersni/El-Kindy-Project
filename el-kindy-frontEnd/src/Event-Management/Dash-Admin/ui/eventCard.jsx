...
import ManageEventParticipantModal from "./ManageEventParticipantModal";

const EventCard = ({ data, onDeleteEvent }) => {
  const [eventList, setEventList] = useState([]);

  useEffect(() => {
    if (data) {
      const fetchEvents = async () => {
        const fetchedEvent = await getEventById(data._id);
        const fetchedUserEvent = await getEventByUserId(data._id);
        // console.log("Fetched Event:", fetchedEvent);
        // console.log('Image URL:', `http://localhost:3000/upload-directory/${fetchedEvent.EventImage}`);

        setEventList(fetchedEvent, fetchedUserEvent);
      };
      ...
      <div key={data._id} className="dash__courses__list__card">
        ...
        <div className="dash__courses__list__card-content">
          <div className="dash__courses__list__card-content-title">
            ...
            <div className="quiz-item-column__2 manage-participants-btn2">
              <div
            ...
            <div className="dash__courses__list__card-content-btns-list">
              <a className="dash__courses__list__card-content-btns-list__show quizes">
                <div onClick={openDetails}>
                  {showDetails && <EventDetailsCard event={data} />}
                  <svg
              ...
              <Link
                to={`/dash-admin-Update-new-event/${data._id}`}
